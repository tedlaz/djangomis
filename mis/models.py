...
from utils.apd_functions import (
    fill_spaces,
    fill_spaces_cut,
    decimal2flat,
    isodate2flat,
...
# from utils.ziputil import create_zip
from utils.ziputil import create_zip_stream
...
class Minas(models.Model):
    ...
    code = models.CharField("Κωδικός Περιόδου", max_length=2, unique=True)
...
class Company(models.Model):
    ...
    name = models.CharField("Όνομα (για φυσικά πρόσωπα)", max_length=30, blank=True)
    patr = models.CharField(
        "Όνομα πατέρα (για φυσικά πρόσωπα)", max_length=30, blank=True
    )
...
class Kpk(models.Model):
    ...
    def kpk_per_test(self, period):
        try:
            return self.kpkapo_set.get(
                apo=self.kpkapo_set.filter(apo__lte=period).aggregate(Max("apo"))[
                    "apo__max"
                ]
            )
        except Exception:
...
class KpkApo(models.Model):
    ...
    perg = models.DecimalField("Ποσοστό εργαζομένου", max_digits=4, decimal_places=2)
    peti = models.DecimalField("Ποσοστό εργοδότη", max_digits=4, decimal_places=2)
    ptot = models.DecimalField("Συνολικό ποσοστό", max_digits=4, decimal_places=2)
...
class EidikotitaKek(models.Model):
    ...
    kpk = models.ForeignKey(Kpk, verbose_name="ΚΠΚ", on_delete=models.PROTECT)
...
class OikKatType(models.Model):
    ...
    oik = models.CharField("Οικογενειακή κατάσταση", max_length=60, unique=True)
...
class Ergazomenos(models.Model):
    ...
    afm = models.CharField("ΑΦΜ", max_length=9, unique=True, validators=[is_afm])
    amka = models.CharField("ΑMKA", max_length=11, unique=True, validators=[is_amka])
...
class Proslipsi(models.Model):
    ...
    apodoxes = models.DecimalField("Αποδοχές", max_digits=8, decimal_places=2)
...
class Parousia(models.Model):
    ...
    minas = models.ForeignKey(Minas, verbose_name="Μήνας", on_delete=models.PROTECT)
...
class Misthodosia(models.Model):
    ...
    def title(self):
        if self.apomina == self.eosmina:
            return (
                f"{self.apomina} {self.etos} ({self.ekdosidate.strftime('%d/%m/%Y')})"
            )
        else:
            return (
                f"{self.mistype} {self.etos} ({self.ekdosidate.strftime('%d/%m/%Y')})"
            )
            ...
            for ptyp, ptypd in prod.items():
                try:
                    formula = Formula.objects.get(
                        part=ptyp, ergt=pro.ergazomenostype, mist=self.mistype
                    )
                except Exception:
                    print(
                        f"formula for {ptyp}, "
                ...
                res[pro][apod_type] = res[pro].get(
                    apod_type, {"apod": 0, "meres": 0, "argia": 0, "apo": "", "eos": ""}
                )
    ...
    def calc_print(self):
        ...
        for pro, dapo in res.items():
            ...
            for typeapo, vals in dapo.items():
                ...
                for eidkek, vls in vals["kratiseis"].items():
                    print(f"    {eidkek} {vls['enos']} {vls['etis']} {vls['total']}")
...
class Apd(models.Model):
    etos = models.IntegerField("Έτος", default=2021)
    minas = models.ForeignKey(Minas, verbose_name="Περίοδος", on_delete=models.PROTECT)
...
class ApdDetails(models.Model):
    apd = models.ForeignKey(Apd, verbose_name="ΑΠΔ", on_delete=models.PROTECT)
...
class FmyDetails(models.Model):
    fmy = models.ForeignKey(Fmy, verbose_name="ΦΜΥ", on_delete=models.PROTECT)
    mis = models.OneToOneField(
        Misthodosia, verbose_name="Μισθοδοσία", on_delete=models.PROTECT
    )
